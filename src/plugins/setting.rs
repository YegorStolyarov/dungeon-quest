...
pub fn load_setting(mut command: Commands) {
    match File::open(SETTING_FILE) {
        Ok(_) => {}
        Err(_err) => create_new_setting_file(),
    };

    let mut file = File::open(SETTING_FILE).expect("Error open file");
    let mut contents = String::new();
    file.read_to_string(&mut contents).expect("Error read file");

    let setting: Setting = serde_json::from_str(&contents).expect("JSON was not well-formatted");
